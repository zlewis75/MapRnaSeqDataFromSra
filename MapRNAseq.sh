...
#SBATCH --mem=15gb
#SBATCH --time=8:00:00
#SBATCH --output=../ParTest/logs/MapRNAseq.%j.out
#SBATCH --error=../ParTest/logs/MapRNAseq.%j.err
...
outdir="/scratch/zlewis/JGI_Sept2022/BatchOne/ParTest"
...
#input file variables
  read1=${fastqPath}/${line}/${line}_1.fastq.gz
  read2=${fastqPath}/${line}/${line}_2.fastq.gz
  unpaired=${fastqPath}/${line}/${line}.fastq.gz
...
#mkdir "${outdir}/TempFiles"
#mkdir "${outdir}/counts"

#make variables for output file names
bam="${outdir}/BamFiles/${line}_"
mkdir ${outdir}/counts/${line}
counts="${outdir}/counts/${line}/${line}_counts.txt"
...
#trim reads
  ...
  trim_galore --illumina --fastqc --length 25 --basename ${line} --gzip -o $trimmed $unpaired
...
#map with STAR
  ...
  --outTmpDir ${outdir}/TempFiles
  --readFilesIn ${line}_trimmed.fq.gz  \
...
# samtools index "$bam"
  ...
  echo "${line} running as PE file only"
	...
	#   module load Trim_Galore/0.6.5-GCCcore-8.3.0-Java-11-Python-3.7.4
	#
	#   trim_galore --illumina --fastqc --paired --length 25 --basename ${line} --gzip -o $trimmed $read1 $read2
	#   wait
	#
	#
	#   #map with STAR
	...
	#     --readFilesIn $trimmed/${line}_val_1.fq.gz $trimmed/${line}_val_2.fq.gz \
	#     --readFilesCommand zcat \
	#     --outSAMtype BAM SortedByCoordinate \
...
#STAR   --runMode genomeGenerate  --genomeSAindexNbases 11    --runThreadN 1   --genomeDir /home/zlewis/Genomes/Neurospora/Nc12_RefSeq/STAR   --genomeFastaFiles /home/zlewis/Genomes/Neurospora/Nc12_RefSeq/GCA_000182925.2_NC12_genomic.fna --sjdbGTFfile /home/zlewis/Genomes/Neurospora/Nc12_RefSeq/GCA_000182925.2_NC12_genomic_GFFtoGTFconversion.gtf
...
#in rare cases there will only be a SRR##_1.fastq.gz format. Use this if nothing else exists.
    ...
    echo "${accesion} running as Read1 file only"
       ...
       trim_galore --illumina --fastqc --length 25 --basename ${line} --gzip -o $trimmed $read1
